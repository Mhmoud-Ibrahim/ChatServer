...
export const registerChatHandlers = async (io, socket) => {
    ...
    socket.on("messages_read", async ({ senderId }) => {
        ...
        try {
            ...
            await MessagesModel.updateMany({ sender: senderId, receiver: receiverId, seen: false }, { $set: { seen: true } });
    ...
    const handlePrivateMessage = async ({ msg, receiverId, imageUrl }) => {
        const senderId = socket.data.userId;
        try {
    ...
    const handleDeleteMessage = async ({ messageId, receiverId }) => {
        const senderId = socket.data.userId;
        try {
    ...
    const handleDeleteSenderMessages = async ({ receiverId }) => {
        const senderId = socket.data.userId;
        try {
    ...
    socket.on("update_profile", async (updatedData) => {
        try {
            ...
            const updatedUser = await User.findByIdAndUpdate(userId, { $set: updatedData }, { new: true } // ليرجع المستخدم بعد التعديل
            );
            if (!updatedUser)
